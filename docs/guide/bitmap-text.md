...
|--------|-----------|--------|
| `RainbowChase` | ![](../assets/reference/palette/rainbow-chase.gif){ width="300" }<br>Rotates a full-spectrum ramp so a rainbow travels through the letters. | multi-hue (the rainbow) |
| `MonoChase` | ![](../assets/reference/palette/mono-chase.gif){ width="300" }<br>A single bright band of one colour chases through the letters — RainbowChase, but monochrome. | `color` (default white) |
| `NeonTubeCrawl` | ![](../assets/reference/palette/neon-tube-crawl.gif){ width="300" }<br>A bright pulse crawls along an otherwise-dim neon tube — one glowing slot moving. | `color` (or set `glow`/`base`) |
| `ChromeSheen` | ![](../assets/reference/palette/chrome-sheen.gif){ width="300" }<br>A dark→bright ramp of one colour with a highlight band that sweeps across — a metallic sheen. | `color` (default silver) |
| `HazardStripes` | ![](../assets/reference/palette/hazard-stripes.gif){ width="300" }<br>An accent colour alternating with a dark ground, marching one slot per step. | `color` + `dark` (or set `a`/`b`) |
